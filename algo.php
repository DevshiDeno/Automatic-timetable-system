...
period1='" . $subjectslots[$i][$j][0] . "<br>" . $aliasslots[$i][$j][0][0] . "',
...
for ($i = 0; $i < $teachers_count; $i++) {
    ...
    for ($j = 0; $j < 6; $j++) {
        $query = "UPDATE" . $database_name . " SET  
period1= '" . $teachers[$i]->days[1][$j][0] . "<br>" . $teachers[$i]->classroom_names[1][$j][0] . "',
...
<script>
    setTimeout(() => {
    window.location.assign("generatetimetable.php?success=true");
