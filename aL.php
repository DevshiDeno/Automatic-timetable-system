...
$dsn = "mysql:host=localhost;dbname=ttms";
$user= "root";
$pwd = "";
...
$queryData = $conn->prepare('SELECT * FROM subjects');
...
for ($I = 0; $I < 2 * $count; $I++) {
    ...
    for ($j = 0; $j < 6; $j++) {
            ...
            isset($teachers[$tindex3]->days[$sem % 2][$j][5])) 
        {
            ...
            // echo "<pre>";
            // // var_dump($teachers[1]->days[$sem % 2][$j][1] . $practicals[$i]->code);
            // echo "</pre>";

            $teachers[1]->days[$sem % 2][$j][2] = $practicals[$i]->code;
            $teachers[2]->days[$sem % 2][$j][3] = $practicals[$i]->code;
            $teachers[3]->days[$sem % 2][$j][2] = $practicals[$i]->code;
            break;
        }
    }
}
